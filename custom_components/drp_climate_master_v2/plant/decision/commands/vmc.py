...
class VmcCommandBuilder:
    ...
    def fill(self, dec: PlantDecision, snapshot: PlantSnapshot, demand: PlantDemandSignals) -> None:
        ...
        # ── P-04: FAN_ONLY — ventilazione pura (§4.1) ───────────────────────────
        # §4.1: in FAN_ONLY nessuna deumidifica, modalità processo VMC = OFF.
        # Il calcolo decisionale è completo (§4.1 + P-03): dec.mode può essere
        # HEATING/COOLING/IAQ_ONLY/VENT_ONLY — irrilevante per questo gate.
        # Eccezione: freecooling è gestito dal suo path dedicato (sotto) perché
        # §4.1 lo consente esplicitamente ("decision gestisce ricircolo/freecooling").
        # Nota: PlantMode.OFF è già intercettato sopra; PlantMode.OFF + FAN_ONLY
        # (vacation/finestre aperte) non raggiunge mai questo branch.
        if dec.gating.user_hvac_mode == HVACMode.FAN_ONLY.value and not demand.vmc_req_free_cooling:
            _fo_dp_sp_c = float(
            ...
            v.power = True
            v.mode = "off"          # §4.1: processo OFF — solo ventilazione, nessun trattamento termico
            v.air_speed = int(_fo_speed)
            v.setpoint_t_c = None   # §4.1: nessun setpoint termico in fan_only
